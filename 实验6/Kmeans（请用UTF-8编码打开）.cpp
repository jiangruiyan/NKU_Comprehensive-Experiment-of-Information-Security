...
// PART1 利用K-Means算法，对训练数据集进行迭代聚类，并创建聚类树
    ...
    // 创建KMeans聚类对象
    pKMeans = new CKMeans(pClusterTree, ++KmeansID, 1, 19);
    // 读取训练集中的记录
...
// PART2 利用聚类树 对测试数据集中的数据进行类型预测，并计算出准确率和混淆矩阵
    ...
    {
        cout << "Read Test File Failed!" << endl;
        return -1;
    }
    // 创建一个混淆矩阵
    ConfuseMatrix* pCfusMatrix = new ConfuseMatrix();
    // 预测正确的记录数设置为0
    iRightRcdNum = 0;
    // 将检测记录数设置为0
    TestRcdNum = 0;
    // 遍历测试数据链表，为每一个数据找到聚类中心
    Rstfile << "======================= Test Results =======================" << endl;
    for(TestListIter = pTestRcdList->begin(); TestListIter != pTestRcdList->end(); TestListIter++)
    {
        ...
        // 更新混淆矩阵
        pCfusMatrix->UpdateValue(pRecord->iLabel, pClusterNode->GetClusterNodeLabel());
        // 将分类记录写到result.txt文件中
        Rstfile << "Record " << TestRcdNum << ": True=" << strTrueLabel << " Pred=" << strPreLabel;
        if(pRecord->iLabel == pClusterNode->GetClusterNodeLabel())
            Rstfile << " Correct" << endl;
        else
            Rstfile << " Wrong" << endl;
    }
    // 打印屏显
    ...
    cout << "Correct predictions: " << iRightRcdNum << endl;
    outfile << "======================= Test Summary =======================" << endl;
    outfile << "Total test records: " << TestRcdNum << endl;
    outfile << "Correct predictions: " << iRightRcdNum << endl;
    // 计算正确分类率
    float fAccuracy = (float)iRightRcdNum / (float)TestRcdNum * 100.0;
    cout << "Accuracy: " << setiosflags(ios::fixed) << setprecision(2) << fAccuracy << "%" << endl;
    outfile << "Accuracy: " << fAccuracy << "\%" << endl;
    Rstfile << "======================= Test Summary =======================" << endl;
    Rstfile << "Total test records: " << TestRcdNum << endl;
    Rstfile << "Correct predictions: " << iRightRcdNum << endl;
    Rstfile << "Accuracy: " << fAccuracy << "\%" << endl;
    // 打印混淆矩阵
    pCfusMatrix->PrintMatrix();
    pCfusMatrix->PrintMatrixToLog();
}
